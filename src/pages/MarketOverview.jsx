    ...
    <div className="bg-[#0f0f12] border border-gray-800 rounded-xl p-6 h-full flex flex-col justify-between">
      ...
      <div className="flex flex-col items-center justify-center py-4 my-2 relative">
        ...
        <div className="relative w-48 h-24 flex justify-center overflow-hidden mb-2">
           ...
           <div className="absolute bottom-0 left-1/2 h-full w-0.5 origin-bottom transition-transform duration-700 ease-out z-20" style={{ transform: `translateX(-50%) rotate(${needleRotation}deg)` }}>
        ...
        <style>{`.mask-radial-gauge { -webkit-mask-image: radial-gradient(circle at bottom center, transparent 55%, black 56%); mask-image: radial-gradient(circle at bottom center, transparent 55%, black 56%); }`}</style>
      </div>
...
// --- 2. Top Movers Component (Faster & Fixed Scroll) ---
const TopMovers = ({ data }) => {
  const [activeTab, setActiveTab] = useState('gainers');
  const gainers = data.filter(item => item.changePer > 0).sort((a, b) => b.changePer - a.changePer).slice(0, 5);
  const losers = data.filter(item => item.changePer < 0).sort((a, b) => a.changePer - b.changePer).slice(0, 5);
  ...
  return (
    <div className="bg-[#0f0f12] border border-gray-800 rounded-xl p-6 h-full flex flex-col overflow-hidden">
      ...
      {/* Added overflow-x-hidden to prevent scrollbar during slide */}
      <div className="space-y-3 flex-1 overflow-y-auto overflow-x-hidden pr-1 scrollbar-none relative">
        <AnimatePresence mode='wait'>
          <motion.div
            ...
            exit={{ opacity: 0, x: -10 }} // Reduced distance
            ...
            {displayData.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/[0.02] transition-colors cursor-pointer group border border-transparent hover:border-gray-800">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-xs font-bold ${activeTab === 'gainers' ? 'text-emerald-400' : 'text-rose-400'}`}>{activeTab === 'gainers' ? '+' : ''}{item.changePer.toFixed(2)}%</p>
                </div>
              </div>
            ))}
          </motion.div>
...
// --- 3. Unified Global Markets Component (Faster & Fixed Scroll) ---
const GlobalMarkets = () => {
...
const MarketOverview = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (currentMinutes >= 9 * 60 + 13 && currentMinutes <= 15 * 60 + 30) fetchAllData();
    }, 5000);
